...
class CoSLAM():
    ...
    def save_ckpt(self, save_path):
        ...
        '''
        save_dict = {'all_planes': self.model.all_planes,'model': self.model.state_dict()}
    ...
    def get_loss_from_ret(self, ret, rgb=True, sdf=True, is_co_sdf=True, depth=True, smooth=False):
        ...
        if sdf:
            if is_co_sdf:
                loss += self.config['training']['sdf_weight'] * ret["co_sdf_loss"] + self.config['training']['fs_weight'] * ret["co_fs_loss"]
            else:
                loss += self.config["mapping"]["w_sdf_fs"] * ret["e_fs_loss"] + self.config["mapping"]["w_sdf_center"] * \
    ...
    def mapping(self, rank):
        ...
        while self.tracking_finished < 1 or self.video.map_counter.value < self.video.counter.value-2:
            self.mapper.run()
    ...
    def terminate(self, rank):
        ...
        print('Save final mesh!')
        keyframe_dict_on_cpu = self.mapper.keyframe_dict
        ...
        # w2w = SE3(self.video.pose_compensate[0].clone().unsqueeze(dim=0)).to(Ps.device)
        # Ps = w2w * Ps.inv()
        #
        # key_estimate_c2w_list = Ps.matrix().data.cpu()
        # pose_save_path = os.path.join(self.config['data']['output'], self.config['data']['exp_name'])
        # np.save(
        #     f'{pose_save_path}/key_est_poses.npy',
        #     key_estimate_c2w_list.numpy(),  # c2ws
        # )
