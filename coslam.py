...
class CoSLAM():
    ...
    def get_loss_from_ret(self, ret, rgb=True, sdf=True, depth=True, fs=True, sdfs=False, smooth=False, depth_var=False):
        '''
    ...
    def global_BA(self, batch, cur_frame_id):
        ...
        if len(self.keyframeDatabase.frame_ids) < 2:
            ...
            if self.config['mapping']['optim_cur']:
                ...
                #print(cur_rot.shape,cur_trans.shape)
    ...
    def create_optimizer(self):
        ...
        trainable_parameters = [{'params': self.model.decoder.parameters(), 'weight_decay': 1e-6, 'lr': self.config['mapping']['lr_decoder']},
                                ...
                                {'params': planes_para, 'eps': 1e-15, 'lr': self.config['mapping']['lr_embed']}]   #lr modify?

        if not self.config['grid']['oneGrid']:
            #trainable_parameters.append({'params': self.model.embed_fn_color.parameters(), 'eps': 1e-15, 'lr': self.config['mapping']['lr_embed_color']})
            trainable_parameters.append({'params': c_planes_para, 'eps': 1e-15, 'lr': self.config['mapping']['lr_embed_color']})

        self.map_optimizer = optim.Adam(trainable_parameters, betas=(0.9, 0.99))
        
        # Optimizer for current frame mapping
        if self.config['mapping']['cur_frame_iters'] > 0:
            params_cur_mapping = [{'params': planes_para, 'eps': 1e-15, 'lr': self.config['mapping']['lr_embed']}]
            if not self.config['grid']['oneGrid']:
                params_cur_mapping.append({'params': c_planes_para, 'eps': 1e-15, 'lr': self.config['mapping']['lr_embed_color']})
    ...
    def run(self):
        ...
        for i, batch in tqdm(enumerate(data_loader)):
            ...
            # Tracking + Mapping
                ...
                # Visualize
                if i % self.config['mapping']['vis'] == 0:

                    self.save_imgs(i, batch['depth'][0], batch['rgb'][0], batch['c2w'][0])
                    print('Visualize', i)
