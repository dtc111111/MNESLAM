...
class Mesher(object):
    ...
    def __init__(self, cfg, SLAM, points_batch_size=500000, ray_batch_size=100000):
        ...
        self.bound = SLAM.bounding_box
        self.marching_cubes_bound = SLAM.marching_cube_bound
    ...
    def get_grid_uniform(self, resolution):
        ...
        """
        bound = self.marching_cubes_bound.cpu()
    ...
    def get_mesh(self, mesh_out_file, keyframe_dict, device='cuda:0', color=True):
        """
        ...
        with torch.no_grad():
            ...
            for i, pnts in enumerate(torch.split(points, self.points_batch_size, dim=0)):
                ...
                z.append(self.model.run_network_flat(pnts.to(device)).cpu().numpy()[:, -1])
            ...
            if color:
                ...
                for i, pnts in enumerate(torch.split(points, self.points_batch_size, dim=0)):
                    ...
                    z_color = self.model.query_color(pnts.to(device)).cpu()[..., :3]
                    z.append(torch.sigmoid(z_color))
